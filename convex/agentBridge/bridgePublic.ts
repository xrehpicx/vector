...
/** Import the visible transcript for a locally discovered provider session. */
export const syncSessionTranscript = mutation({
  args: {
    deviceId: v.id('agentDevices'),
    deviceSecret: v.string(),
    liveActivityId: v.id('issueLiveActivities'),
  ...
  handler: async (ctx, args) => {
    ...
    if (args.messages.length > 50) {
      throw new ConvexError('TRANSCRIPT_BATCH_TOO_LARGE');
    }
    ...
    if (title && activity.workSessionId) {
      const workSession = await ctx.db.get(
        'workSessions',
        activity.workSessionId,
      );
      if (workSession && !workSession.titleLockedByUser) {
        await ctx.db.patch('workSessions', workSession._id, { title });
      }
    }
    ...
    for (const message of args.messages) {
      ...
      const existing = await ctx.db
        ...
        .withIndex('by_live_activity_and_source_id', q =>
          q
            .eq('liveActivityId', args.liveActivityId)
            .eq('sourceId', message.sourceId),
        )
        .first();
      const direction =
        message.role === 'user' ? 'vector_to_agent' : 'agent_to_vector';
      const deliveryStatus = message.role === 'user' ? 'delivered' : 'sent';
      ...
      if (existing) {
        if (
          ...
          await ctx.db.patch('issueLiveMessages', existing._id, {
            body,
            role: message.role,
            direction,
            deliveryStatus,
